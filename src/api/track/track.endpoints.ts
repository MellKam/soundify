 ...
 */
export const saveTracks = async (client: HTTPClient, track_ids: string[]) => {
 ...
 * @param track_id Spotify track ID
 */
export const checkSavedTrack = async (client: HTTPClient, track_id: string) => {
...
export const getTracksAudioFeatures = async (
  ...
  return (
    await client.fetch<{ audio_features: AudioFeatures[] }>(
      "/audio-features",
      "json",
      {
        query: {
